   ...
   $.widget("ui.weekCalendar", {
       ...
       options: {
         ...
         eventAfterRender : function(calEvent, element) {
			  //element.find(".wc-time").html('Title of the time-label-field');
            return element;
         },
...
//            options.millisPerTimeslot = options.millisToDisplay / options.timeslotsPerDay;
            ...
            for (var s = 0; s < timeSlots.length; s++) {
                ...
                totalHours += timeSlots[s].end - timeSlots[s].start;
            }
           // totalHours = totalHours == 0 ? 1 : totalHours;
           // alert('totalHours =' + totalHours);
             options.timeslotsPerDay = options.timeslotsPerHour * totalHours;
             //alert( options.timeslotsPerDay + '  ' + options.millisToDisplay + '  ' + options.millisPerTimeslot);
             options.millisToDisplay = totalHours * 60 * 60 * 1000;
             options.millisPerTimeslot = options.millisToDisplay / options.timeslotsPerDay;
...
//         }
        ...
        for ( var s=0;s<timeSlots.length;s++){
             for (var i = timeSlots[s].start; i < timeSlots[s].end; i++) {
                for (var j = 0; j < options.timeslotsPerHour - 1; j++) {
                   calendarBodyHtml += "<div class=\"wc-time-slot\"></div>";
                }
                calendarBodyHtml += "<div class=\"wc-time-slot wc-hour-end\"></div>";
...
//         }
          ...
          for ( var s=0;s<timeSlots.length;s++){
                //alert(timeSlots[s].toString());
             for (var i = timeSlots[s].start; i <timeSlots[s].end; i++) {
               // alert(timeSlots[s].start+ '  ' + timeSlots[s].end + '  i='+i);
                var bhClass = (timeSlots[s].start <= i && timeSlots[s].end > i) ? "wc-business-hours" : "";
                calendarBodyHtml += "<div class=\"wc-hour-header " + bhClass + "\">"
                if (options.use24Hour) {
                   calendarBodyHtml += "<div class=\"wc-time-header-cell\">" + self._24HourForIndex(i) + "</div>";
                } else {
                   calendarBodyHtml += "<div class=\"wc-time-header-cell\">" + self._hourForIndex(i) + "<span class=\"wc-am-pm\">" + self._amOrPm(i) + "</span></div>";
                } //alert(self._24HourForIndex(i));
                calendarBodyHtml += "</div>";
             }
      ...
      _positionEvent : function($weekDay, $calEvent) {
         ...
         for (var s = 0; s < timeSlots.length; s++) {
             if (calEvent.start.getHours() >= timeSlots[s].start && calEvent.end.getHours() <= timeSlots[s].end) {
                 if (s == 0) {
                     firstHourDisplayed = timeSlots[s].start;
                 }
                 else {
                     var tempHour = 0;
                     for (var j = s; j>0; j--) {
                         tempHour +=  (timeSlots[j - 1].end - timeSlots[j - 1].start);
                     }
                     //alert(tempHour);
                     firstHourDisplayed = timeSlots[s].start - tempHour;
                 }
                 break;
             }
         }
        ...
        $calEvent.css({ top: pxTop, height: pxHeight });
         ...
         //var firstHourDisplayed = options.businessHours.limitDisplay ? options.businessHours.start : 0;
         //alert('Date= '+new Date(calEvent.start.getFullYear(), calEvent.start.getMonth(), calEvent.start.getDate(), 17));
         //var startMillis = calEvent.start.getTime() - new Date(calEvent.start.getFullYear(), calEvent.start.getMonth(), calEvent.start.getDate(), 17).getTime();
         //var eventMillis = calEvent.end.getTime() - calEvent.start.getTime();
         //var pxTop = pxPerMillis * startMillis;
         //var pxHeight = pxPerMillis * eventMillis;
         //$calEvent.css({top: pxTop, height: pxHeight});
      ...
      _refreshEventDetails : function(calEvent, $calEvent) {
         ...
         if (displayTitleWithTime){
             ...
             if (this.options.isPatient) {
                 if (this.options.isClinic ==false) {                    
                     $calEvent.find(".wc-time").html('<a id="' + calEvent.id + '" href="' + this.options.appointmentURL + calEvent.id + '">' + self._formatDate(calEvent.start, options.timeFormat) + '</a>');
                 }
                 else {
                     $calEvent.find(".wc-time").html('<a id="' + calEvent.id + '" href="' + this.options.appointmentURL + calEvent.id + '">' + self._formatDate(calEvent.start, options.timeFormat) + '</a>');
